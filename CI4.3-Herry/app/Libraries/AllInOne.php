...
//原綠界不支援namebase 改寫sdk
//如果要增加付款方式  需自行添加function
//uksort 下 ECPay_CheckMacValue 須改為 App\Libraries\ECPay_CheckMacValue
//throw new Exception 須改為 throw new /Exception
...
//    public $EncryptType = ECPay_EncryptType::ENC_MD5;
    public $EncryptType = ECPay_EncryptType::ENC_SHA256;
    ...
    function CheckOutFeedback() {
        return $arFeedback = ECPay_CheckOutFeedback::CheckOut(array_merge($_POST, array('EncryptType' => $this->EncryptType)),$this->HashKey,$this->HashIV);
    }
...
//        self::$PaymentObj = new $PaymentMethod;
		//原綠界不支援namebase 改寫sdk
		if($arParameters['ChoosePayment'] == "ATM")
		{
			self::$PaymentObj = new ECPay_ATM;
		}
		else if($arParameters['ChoosePayment'] == "Credit")
		{
			self::$PaymentObj = new ECPay_Credit;
		}
		else if($arParameters['ChoosePayment'] == "CVS")
		{
    ...
    class ECPay_CheckMacValue{
        ...
        public static function generate($arParameters = array(),$HashKey = '' ,$HashIV = '',$encType = 0){
            ...
            if(isset($arParameters))
            {
                ...
                uksort($arParameters, array('App\Libraries\ECPay_CheckMacValue','merchantSort'));
