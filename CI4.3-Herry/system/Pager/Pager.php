...
class Pager implements PagerInterface
{
     ...
     * @return string|URI
     */
    public function getPageURI(?int $page = null, string $group = 'default', bool $returnObject = true)
    {
        $this->ensureGroup($group);
    ...
    protected function calculateCurrentPage(string $group)
    {
        if (array_key_exists($group, $this->segment)) {
            try {
				//橘野修改 為了使base_url能吃到資料夾
				$segmentNum = $this->segment[$group] + $_SERVER['GS_ENVIRONMENT']['pager_base_url'];
				
                $this->groups[$group]['currentPage'] = (int) $this->groups[$group]['currentUri']
                    ->setSilent(false)->getSegment($segmentNum);
